...
const metrics = computed(() => [
  ...
  { label: 'Estimated model cost', value: formatUsd(summary.value?.estimatedCostUsd ?? 0), note: `${(summary.value?.totalTokens ?? 0).toLocaleString()} tokens · recent turns` },
])
      ...
      <article class="panel runs-panel">
        ...
        <div class="run-list">
          <div v-for="run in runs?.runs" :key="run.id" class="run">
            ...
            <div v-if="run.inputTokens != null || run.outputTokens != null" class="usage"><strong>{{ formatUsd(run.estimatedCostUsd ?? 0) }}</strong><span>estimated</span><span>{{ (run.inputTokens ?? 0).toLocaleString() }} in</span><span>{{ (run.outputTokens ?? 0).toLocaleString() }} out</span><span v-if="run.cachedInputTokens">{{ run.cachedInputTokens.toLocaleString() }} cached</span></div>
...
<style scoped>
.usage { display: flex; align-items: baseline; flex-wrap: wrap; gap: 5px 10px; margin-top: 9px; font-size: 10px; color: var(--muted); }
.usage strong { color: var(--green); font: 700 15px 'Manrope'; }
